...
Installers are **never committed** — they ship as **GitHub Release assets**. `.github/workflows/release.yml` builds them on a matrix (`ubuntu-22.04` for portable `.deb`/`.rpm`/`.AppImage`, `windows-latest` for NSIS `.exe` + MSI) via `tauri-action`, on every `app-v*` tag. Cut a release with the **`/release`** command (`.claude/commands/release.md`): it bumps the version across `tauri.conf.json`/`package.json`/`Cargo.toml`/`Cargo.lock`, prepends a generated section to `CHANGELOG.md` (a curated, user-facing French summary of the changes since the last tag — internal noise omitted, internal categories collapsed to one generic line), runs the gate, commits, tags `app-v<version>`, and pushes. The pipeline then creates a **draft** Release at `…/releases`, using that `CHANGELOG.md` section as the description (CI extracts it by version and appends the download line), for you to review and publish. Build releases only through CI (`ubuntu-22.04`) — never from a dev box, whose newer glibc breaks portability on older distros.
